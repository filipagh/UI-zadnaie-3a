...
public class Ui {
	...
	{
		populacia = 20;		//urcuje pocet jedincov 
	...
	public static void vlozdostromu(Stav a,int skore)  // vlozenie stavu a jeho vahy do stromu
	{
		tmap.put(skore,a);
	}

	public static void vytvor(int n)  // metoda na generovanie 1. generacie
	{
		for (int p=0;p<n;p++)   // n*populacia
		{
			int[] geny =new int[45];       // pole vsetkych vstupov
			int[] geny_exp = new int[28];  // genon
			for (int i=1;i<45;i++)   // vytvorenie postupnosti  1 2 3 4 .... 43 44 
			{
				geny[i]=i;
			}
			Random rand =new Random();
			for (int i=0;i<200;i++)   // kazdy gen nahodne poprehadzujem
			{
			...
			for (int i=1;i<28;i++)  // prepis prvych 27 genov
			{
				geny_exp[i]=geny[i];
			}
			vloz(new Stav(geny_exp));    // vlozenie stavu do zoznamu akt generaciqe
		}
	}


	// metody na vlozenie do zoznamu
